...
import com.ai.paas.ipaas.uac.constants.UserClientException;
...
public class UserClientImpl implements IUserClient {
	...
	public AuthResult authUser(AuthDescriptor ad)  {
		...
		if(ad == null){
			throw new UserClientException(PaaSConstant.ExceptionCode.USER_AUTH_ERROR,"AuthDescriptor is null");
		}else{
		...
		if (StringUtil.isBlank(username)
		 ...
		 || StringUtil.isBlank(serviceId)) {
			throw new UserClientException(PaaSConstant.ExceptionCode.USER_AUTH_ERROR,"username or password or authAdress or serviceId is blank");
		}
		String param = "password=" + password + "&authUserName=" + username + "&serviceId=" + serviceId;
		...
		if(successed){
			...
			throw new UserClientException(PaaSConstant.ExceptionCode.USER_AUTH_ERROR,"username or password is wrong!");
		}
		return ar;
	}
	...
	public AuthResult auth(AuthDescriptor ap) {
		...
		if (StringUtil.isBlank(password)
		 || StringUtil.isBlank(pid)
		 || StringUtil.isBlank(authAdress)
		 || StringUtil.isBlank(serviceId)) {
			throw new UserClientException(PaaSConstant.ExceptionCode.USER_AUTH_ERROR,"password or authAdress or serviceId or pId is blank");
		}
		String param = "password=" + password + "&pId=" + pid + "&serviceId=" + serviceId;
		String postRes = HttpRequestUtil.sendPost(authAdress, param);
